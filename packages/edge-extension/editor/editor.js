// Editor con características específicas de Edge
let edgeFeatures = {};
let pyodide = null;
...
function addEdgeCollectionsButton() {
  const toolbar = document.querySelector('.toolbar-right');
  const button = document.createElement('button');
  button.id = 'btn-collections';
  button.className = 'toolbar-btn';
  button.title = 'Guardar en Collections';
  button.innerHTML = '<i class="fas fa-layer-group"></i>';
  button.addEventListener('click', saveToCollections);
...
async function saveToCollections() {
  ...
  if (response.success) {
    addLog('success', 'Guardado en Edge Collections');
  }
}
...
async function compileWithPyodide() {
  setTimeout(() => {
    const fakePdf = new Blob(
      ['CV generado correctamente en Edge'],
